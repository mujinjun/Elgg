...
function cnsns_theme_init() {
	/**
	 * Customize pages
	 */
	elgg_register_plugin_hook_handler('index', 'system', 'cnsns_theme_index_handler');
	elgg_register_page_handler('profile', 'cnsns_theme_profile_page_handler');
	elgg_register_page_handler('dashboard', 'cnsns_theme_dashboard_handler');

	//What a hack!  Overriding groups page handler without blowing away other plugins doing the same
	global $CONFIG, $cnsns_theme_original_groups_page_handler;
	$cnsns_theme_original_groups_page_handler = $CONFIG->pagehandler['groups'];
	elgg_register_page_handler('groups', 'cnsns_theme_groups_page_handler');
	...
	elgg_register_plugin_hook_handler('register', 'menu:river', 'cnsns_theme_river_menu_handler');
	elgg_register_plugin_hook_handler('register', 'menu:owner_block', 'cnsns_theme_owner_block_menu_handler', 600);
	elgg_register_plugin_hook_handler('register', 'menu:composer', 'cnsns_theme_composer_menu_handler');

	elgg_register_event_handler('pagesetup', 'system', 'cnsns_theme_pagesetup_handler', 1000);

	/**
	 * Customize permissions
	 */
	elgg_register_plugin_hook_handler('permissions_check:annotate', 'all', 'cnsns_theme_annotation_permissions_handler');
	elgg_register_plugin_hook_handler('container_permissions_check', 'all', 'cnsns_theme_container_permissions_handler');

	/**
	 * Miscellaneous customizations
	 */
	//Small "correction" to groups profile -- brief description makes more sense to come first!
	elgg_register_plugin_hook_handler('profile:fields', 'group', 'cnsns_theme_group_profile_fields', 1);

	//@todo report some of the extra patterns to be included in Elgg core
	elgg_extend_view('css/elgg', 'cnsns_theme/css');
...
function cnsns_theme_groups_page_handler($segments, $handle) {
	...
	switch ($segments[0]) {
		...
		case 'discussion':
			...
			return true;
		default:
			global $cnsns_theme_original_groups_page_handler;
			return call_user_func($cnsns_theme_original_groups_page_handler, $segments, $handle);
	}
}

function cnsns_theme_pagesetup_handler() {
...
function cnsns_theme_dashboard_handler() {
	require_once dirname(__FILE__) . '/pages/dashboard.php';
	return true;
}

function cnsns_theme_index_handler() {
	if (elgg_is_logged_in()) {
		forward('/dashboard');
	}
}

function cnsns_theme_container_permissions_handler($hook, $type, $result, $params) {
...
function cnsns_theme_annotation_permissions_handler($hook, $type, $result, $params) {
 ...
 */
function cnsns_theme_composer_menu_handler($hook, $type, $items, $params) {
	$entity = $params['entity'];
...
function cnsns_theme_group_profile_fields($hook, $type, $fields, $params) {
	return array(
...
function cnsns_theme_owner_block_menu_handler($hook, $type, $items, $params) {
	$owner = elgg_get_page_owner_entity();
...
function cnsns_theme_river_menu_handler($hook, $type, $items, $params) {
	$item = $params['item'];
 ...
 */
function cnsns_theme_profile_page_handler($page) {
	global $CONFIG;
...
elgg_register_event_handler('init', 'system', 'cnsns_theme_init');